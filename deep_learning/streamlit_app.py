...
# försök för att göra första inladdnigen snabbare genom att cacha data. Inte helt hundra hur det funkar
@st.cache_data
def load_data():
    return pickle.load(open("texts.pkl", "rb")), faiss.read_index("faiss_index.bin")

texts, index = load_data()
sentence_transformer_model = SentenceTransformer("paraphrase-MiniLM-L6-v2") # modell för att läsa in subtitles (klassifiecring osv)

config = configparser.ConfigParser()
config.read("config.ini") # läs config för att senare ta fram nyckeln
api_key = config.get("api", "key").strip('"') 
# hämta nyckeln som inte syns på github samt ta bort citat tecken från variablen


def get_relevant_context(query, top_n=3):
    query_vector = sentence_transformer_model.encode([query])
...
async def generate_answer(query, api_key):  # kallar genai, retunerar svar
    try:
        relevant_info = get_relevant_context(query) # använd sentence transfomer för att få context
        context_str = "\n".join(relevant_info)
        ...
        gemini_model = genai.GenerativeModel("gemini-2.0-flash") # "byta" model till gemini då vi ej behöver sentence transformer längre
        # prompt för att instruera gemini
        ...
        Question: {query}"""
        response = await asyncio.to_thread(gemini_model.generate_content, prompt) # async kör generate_content (http request) från model klassen
